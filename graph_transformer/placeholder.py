...
def er_validation_step(val_dataloader, val_metric, er_model):
    ...
    for batch in val_dataloader:
        ...
        pred = PlaceHolder(X=target_holder.X, E=er_E_pred, y=target_holder.y).mask(node_mask)
...
def to_dense(data_batch, E_num_classes=2):
    ...
    E = to_dense_adj(edge_index=edge_index, batch=batch, edge_attr=edge_attr, max_num_nodes=20)
    E = E + torch.transpose(E, 1, 2)

    E = torch.nn.functional.one_hot(E.long().squeeze(-1), E_num_classes).float()
    unmasked_holder = PlaceHolder(X=X, E=E, y=None)  # not masking because then we are sampling from multinomial
